...
if page == "Задание 1":
    ...
    st.session_state.p_0_d = f"{point_0_d.h:.2f}"
    p_1t = ppp + delta_p_pp
...
if page == "Задание 3":
    ...
    P0 = st.number_input('Введите давление полного торможения перед нерегулируемой ступенью P0_, МПа', round(point_0_d.P,2))
    h0 = st.number_input('Введите энтальпия полного торможения перед первой нерегулируемой ступенью h0_, кДж/кг', value=3343.96)
    Pz = st.number_input('Введите давление за ЦВД, МПа ', value=4.4)
    G0 = st.number_input('Введите расход пара в первую нерегулируемую ступень G0, кг/с', value=429.21)
    drs = st.number_input('Введите диаметр регулирующей ступени, м', value=1.1)
    etaoi = st.number_input('Введите внутренний КПД ЦВД ηол', value=0.799, max_value=1.000)
...
if page == "Задание 3":
    ...
    P0 = st.number_input('Введите давление полного торможения перед нерегулируемой ступенью P0_, МПа', value=22.99)
    h0 = st.number_input('Введите энтальпия полного торможения перед первой нерегулируемой ступенью h0_, кДж/кг', value=3348.57)
    Pz = st.number_input('Введите давление за ЦВД, МПа ', value=4.4)
    G0 = st.number_input('Введите расход пара в первую нерегулируемую ступень G0, кг/с', value=439.332)
    drs = st.number_input('Введите диаметр регулирующей ступени, м', value=1.1)
    etaoi = st.number_input('Введите внутренний КПД ЦВД ηол', value=0.799, max_value=1.000)
